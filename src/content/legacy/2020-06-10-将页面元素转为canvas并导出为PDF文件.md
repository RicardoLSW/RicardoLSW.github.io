...
date: 2020-06-10
description: "前言 这几天在做一个电子收据系统的项目，涉及到收据开具、审核、收款、打印这些功能，说道收款，就想起来前两天测试扫码付款导致我血亏7分钱巨款~诶~不提了，今天这篇主要说一下收据打印功能，收据收款完成后，需要导出PDF用于打印盖章。 原型图 代"
categories:
  - 前端
tags:
  - Vue
  - JavaScript
legacyPath: "/前端/将页面元素转为canvas并导出为PDF文件/"
lastModified: "2020-06-10 19:34:44"
---
...
![image-20200611075929124](https://figure-b.ricardolsw.com/image/7dSzPNKpl16QTz6xxv4NDoS2qYxmPnsj.jpg?x-oss-process=style/watermark)
...
1. 先把页面给弄出来，这里我用的Vue和Ant Design组件库：
   ...
   ![image-20200611075817185](https://figure-b.ricardolsw.com/image/c8BB3xcFb7KMtOc2WiUGbPRo0ye6Uoww.jpg?x-oss-process=style/watermark)
...
2. 接下来需要把收据这部分导出为PDF文件，这里用到了两个插件，分别是html2canvas和jspdf，其中html2canvas可以把html元素转换成canvas，jspdf可以把图片文件转换成PDF文件，而刚好canvas可以生成图片文件，并且把html转成canvas还可以做到防篡改，无法通过审查元素修改其收据内容，还能给上头加些水印啥的~
   ...
   ![image-20200611075848765](https://figure-b.ricardolsw.com/image/62UzEDmbUAE3FGucf08TxSTLN4EwR87f.jpg?x-oss-process=style/watermark)
